...
#         passwrd = request.POST.get('password', '').strip()
...
#         try:
#             imap_url = 'imap.gmail.com'  # Assuming Gmail for this example
#             conn = imaplib.IMAP4_SSL(imap_url)
#             print(conn)
#             conn.login(addr, passwrd)
#             print(conn.login(addr,passwrd))
#             # Successful login
#             request.session['email'] = addr  # Storing email in session for future use
...
#     # If request.method != 'POST', render the login page
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None and user.check_password(password):
            ...
            return redirect('homepage:login')  # Redirect back to the login page
    else:
    ...
    #     # Assuming convert_special_char() is a valid function to sanitize email
    #     addr = convert_special_char(addr)
    #     # print(flag)`1`
    #     # while (flag):
...
def options_view(request):
    global i, addr, passwrd,newtoaddr
    if request.method == 'POST':
        # flag = True
...
def compose_view(request):
    if request.method == 'POST':
        sender_email = "ashilstartup@gmail.com"
...
#     global i, addr, passwrd, s, item, subject, body,newtoaddr
#     if request.method == 'POST':
...
#         #     print(item)
#         newtoaddr = "arunchandranvk84@gmail.com"
#         msg = MIMEMultipart()
#         msg['From'] = fromaddr
#         msg['To'] = newtoaddr
#         # flag = True
...
#             # i = i + str(1)
#             return redirect('homepage:compose')
#         #     return JsonResponse({'result': 'failure'})
#         # s.quit()
#         # return JsonResponse({'result' : 'success'})
    
#     compose  = Compose()
#     compose.recipient = newtoaddr
...
# sender_email = "donkannukkadan@gmail.com"
# receiver_email = "arundemo4@gmail.com"
# subject = "Test email"
# message = "Hello, this is a test email!"
# smtp_server = "smtp.gmail.com"
# smtp_port = 587  # Use 587 for TLS connection
# smtp_username = "donkannukkadan@gmail.com"
# smtp_password = "ksqh kvui hlsh igul"
...
#     imap_port = 587
#     username = 'jipsongeorge753@gmail.com'
#     password = 'oyhzlplyolihopys'
...
#         # Loop through the email IDs returned by the search
...
#             # Extract email details (e.g., sender, subject, date)
#             sender = msg['From']
#             subject = msg['Subject']
#             date = msg['Date']
...
#         mail.close()
#         mail.logout()
#         return render(request, 'homepage/inbox.html', {'emails': emails})
#     except Exception as e:
#         # Handle exceptions and print the error message
        
#         return redirect('homepage:options')
    
def read_received_emails(request):
    # Connect to the IMAP server
    imap_server = "imap.gmail.com"
    username = 'ashilstartup@gmail.com'
    password = 'iahm gpjc kqph tumu'
    ...
    sent_emails.sort(key=lambda x: email.utils.parsedate(x['date']), reverse=True)

    # Render the template with the sent emails
    return render(request, 'homepage/inbox.html', {'sent_emails': sent_emails})

from imapclient import IMAPClient
    
def read_sent_emails(request):
    # Connect to the IMAP server
    imap_server = "imap.gmail.com"
    username = 'ashilstartup@gmail.com'
    password = 'iahm gpjc kqph tumu'
    ...
    try:
        ...
        with IMAPClient(imap_server) as client:
            ...
            # Retrieve list of available mailboxes
            mailboxes = client.list_folders()

            # Find the correct mailbox name for sent emails
            sent_mailbox_name = None
            for flags, delimiter, mailbox_name in mailboxes:
                if b'\Sent' in flags:
                    sent_mailbox_name = mailbox_name
                    break

            if not sent_mailbox_name:
                raise Exception("Sent mailbox not found")

            # Select the mailbox for sent emails
            client.select_folder(sent_mailbox_name)
    ...
    # Sort all emails based on date in descending order
        sent_emails.sort(key=lambda x: email.utils.parsedate(x['date']), reverse=True)

    # Render the template with the sorted emails
    return render(request, 'homepage/sent.html', {'sent_emails': sent_emails})
    
def read_trash_emails(request):
    # Connect to the IMAP server
    imap_server = "imap.gmail.com"
    username = 'ashilstartup@gmail.com'
    password = 'iahm gpjc kqph tumu'
    ...
    # Render the template with the sorted emails
    return render(request, 'homepage/trash.html', {'sent_emails': sent_emails})
